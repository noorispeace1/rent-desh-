...
export default function MyBookingsPage() {
  ...
  const fetchData = async (userId) => {
    try {
      setLoading(true);
      // Note: Currently fetching all properties as placeholders for bookings
      const propsRes = await fetch('http://localhost:5000/properties', { cache: 'no-store' });
      if (propsRes.ok) {
        const propsData = await propsRes.json();
        setBookings(propsData);
        setFilteredBookings(propsData);
      }

      // Fetch favorite IDs
      const favsRes = await fetch(`http://localhost:5000/favorites/ids/${userId}`, { cache: 'no-store' });
      if (favsRes.ok) {
  ...
  const handleFilter = (filters) => {
    const filtered = bookings.filter((property) => {
    ...
    <div className="w-full max-w-[1200px] mx-auto p-4 sm:p-6 lg:p-8">
      ...
      {filteredBookings.length === 0 ? (
        <div className="bg-white border border-[#E5E7EB] rounded-2xl p-12 text-center flex flex-col items-center justify-center min-h-[400px]">
          ...
          <h3 className="text-lg font-semibold text-[#1C1C1E] mb-2">No bookings found</h3>
          <p className="text-[#6B7280] text-sm max-w-sm mb-6">You haven't booked any properties yet or try refining your filters. Explore our listings to find your next home.</p>
          <Link 
            href="/properties"
          ...
          {filteredBookings.map((property) => (
            <div key={property._id} className="group bg-white rounded-2xl border border-[#E5E7EB] overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
              <div className="relative h-[240px] w-full bg-gray-100 overflow-hidden">
                ...
                <div className="absolute top-4 right-4 bg-indigo-600 px-3 py-1 rounded-full text-xs font-bold text-white shadow-sm">
                  Booked
                </div>
                <FavoriteButton 
