...
async function getFavoriteIds(userId) {
  if (!userId) return [];
  try {
    const res = await fetch(`http://localhost:5000/favorites/ids/${userId}`, { cache: 'no-store' });
    if (!res.ok) return [];
    return res.json();
  } catch (error) {
    ...
    <div className="bg-[#F9FAFB] min-h-screen pb-16">
      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 mt-16">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
          <div>
            <h2 className="text-3xl font-black text-[#1C1C1E] tracking-tight mb-2">Featured Properties</h2>
            <p className="text-[#6B7280] text-sm">Explore our handpicked premium and verified listings across Bangladesh.</p>
          </div>
          <Link 
            href="/properties" 
            className="inline-flex items-center justify-center bg-[#009282] hover:bg-[#007A6C] text-white px-6 py-3 rounded-xl font-bold text-sm transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5"
          >
        ...
        {featuredProperties.length === 0 ? (
          <div className="bg-white border border-[#E5E7EB] rounded-2xl p-12 text-center flex flex-col items-center justify-center min-h-[300px]">
            ...
            <h3 className="text-lg font-semibold text-[#1C1C1E] mb-2">No properties listed yet</h3>
            <p className="text-[#6B7280] text-sm max-w-sm mb-4">Be the first to list a property on RentDesh.</p>
          </div>
        ) : (
